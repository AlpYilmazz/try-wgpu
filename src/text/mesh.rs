use crate::resource::{mesh::Mesh, buffer::Vertex};
...
pub fn create_screen_text_mesh(atlas: &TextAtlas, src: &str, coord: (f32, f32)) -> Mesh<Vertex> {
    ...
    for ch in src.chars() {
        ...
        vertices.extend(&[
            Vertex { position: [x_start, y_start + h, 0.0], tex_coords: [tl.0, tl.1] }, // tl
            Vertex { position: [x_start, y_start, 0.0], tex_coords: [tl.0, br.1] }, // bl
            Vertex { position: [x_start + w, y_start, 0.0], tex_coords: [br.0, br.1] }, // br
            
            Vertex { position: [x_start + w, y_start, 0.0], tex_coords: [br.0, br.1] }, // br
            Vertex { position: [x_start + w, y_start + h, 0.0], tex_coords: [br.0, tl.1] }, // tr
            Vertex { position: [x_start, y_start + h, 0.0], tex_coords: [tl.0, tl.1] }, // tl
        ]);
